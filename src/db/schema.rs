...
mod tests {
    ...
    // Contract fixture and column assertions below describe the pre-handles schema.
    // `fixtures/schema/current-schema.json` is regenerated by
    // `node scripts/sync-vault-schema.mjs` and these assertions (plus the
    // `chat_identifier`-based inserts in `setup()`) are updated to the new schema
    // in a later task of the handle-identity-model plan; until then this test and
    // the tests using `setup()` fail against the new DDL.
    #[test]
