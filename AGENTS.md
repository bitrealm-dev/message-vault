...
|-----------------|---------------------|----------------------------------------------------------------------------------------|
| Product version | `0.8.0`             | Desktop app + vault image. Git tag is `v0.8.0`.                                        |
| Docker Hub tag  | `0.8.0` (no `v`)    | `bitrealm/message-vault:0.8.0`. Also `0.8`, `latest`, and `sha-…`.                     |
| JSONL schema    | `schema_version: 3` | Shared chat file format. Independent of the product version. Do not invent v2 readers. |

**Product version files** (keep these in lockstep; current value is `0.8.0`):
