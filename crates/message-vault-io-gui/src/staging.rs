...
//!
//! Guided import writes JSONL (one JSON object per line) and attachments into
//! `staging-<importer>-…` beside `export.ini`.
//! Vault export writes into `export-<type>-…` under the chosen parent folder.
//! When that parent is empty, the process working directory is used.

use std::path::{Path, PathBuf};

use chrono::{DateTime, Local};

/// Short name used in staging directory names for iPhone / iOS backups.
pub const IPHONE_IOS_IMPORTER: &str = "iphone-ios";

/// Short name used in staging directory names for macOS Messages (`chat.db`).
pub const MACOS_IMPORTER: &str = "macos";

/// Short name used in Vault Export directory names (iMessage covers iOS and macOS).
pub const IMESSAGE_EXPORTER: &str = "imessage";

/// Directory that holds `export.ini`, or `.` if the path has no parent.
pub fn export_ini_parent_dir(export_ini_path: &Path) -> PathBuf {
    export_ini_path
        .parent()
...
/// Staging directory next to `export.ini`.
pub fn staging_dir_path(export_ini_path: &Path, importer: &str, now: DateTime<Local>) -> PathBuf {
    export_ini_parent_dir(export_ini_path).join(staging_dir_name(importer, now))
}
...
/// Parent folder for Vault Export when the UI field is empty: the process working directory.
