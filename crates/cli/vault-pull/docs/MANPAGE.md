...
Generate an Import API token under **Settings → Profile** in the vault UI. The vault base URL is the same origin as the web UI (for example `http://127.0.0.1:8080`).
...
| `--key` | `VAULT_KEY` | Per-account Import API token |
