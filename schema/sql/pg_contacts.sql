-- Postgres variant of contacts.sql (the SQLite original). `id` columns are
-- BIGINT identities; `datetime('now')` defaults become UTC timestamps in the
-- same `YYYY-MM-DD HH:MM:SS` format (see dialect::now_utc_sql).
...
CREATE TABLE IF NOT EXISTS contacts (
    ...
    last_modified TEXT NOT NULL DEFAULT (to_char(now() AT TIME ZONE 'UTC', 'YYYY-MM-DD HH24:MI:SS'))
);
...
CREATE TABLE IF NOT EXISTS trashed_handles (
    ...
    trashed_at TEXT NOT NULL DEFAULT (to_char(now() AT TIME ZONE 'UTC', 'YYYY-MM-DD HH24:MI:SS')),
    PRIMARY KEY (account_id, handle_id)
);
