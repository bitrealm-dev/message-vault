-- Postgres variant of accounts.sql (the SQLite original). `id` columns are
-- BIGINT identities; COLLATE NOCASE has no Postgres twin, so case-insensitive
-- uniqueness moves to expression indexes on lower(...).
-- Vault login account (web UI + API owner).
CREATE TABLE IF NOT EXISTS accounts (
    -- Stable account id (opaque string primary key).
    id TEXT PRIMARY KEY,
    -- Login user id; unique case-insensitively via ix_accounts_username_ci.
...
-- Case-insensitive username uniqueness (SQLite's UNIQUE COLLATE NOCASE twin).
CREATE UNIQUE INDEX IF NOT EXISTS ix_accounts_username_ci ON accounts (lower(username));

-- Email addresses attached to an account (not used for login).
CREATE TABLE IF NOT EXISTS account_emails (
    -- Owning vault account (`accounts.id`).
    account_id TEXT NOT NULL REFERENCES accounts(id) ON DELETE CASCADE,
    -- Email address; unique case-insensitively via ix_account_emails_email_ci.
    email TEXT NOT NULL,
...
-- Case-insensitive email uniqueness (SQLite's UNIQUE COLLATE NOCASE twin).
CREATE UNIQUE INDEX IF NOT EXISTS ix_account_emails_email_ci ON account_emails (lower(email));
