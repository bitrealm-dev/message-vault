-- Postgres variant of messages.sql (the SQLite original). Every `id` column
-- is a BIGINT identity; child FK columns stay INTEGER (Postgres allows an
-- integer FK to reference a bigint primary key). FTS is SQLite-only — the
-- Postgres full-text twin lands separately.
