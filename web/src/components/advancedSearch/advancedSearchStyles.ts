...
/**
 * Select triggers in this panel sit on the panel background rather than the
 * field background. Everything else about their size now comes from
 * `<Select size="sm">`, so no `!important` overrides are needed here.
 */
export const selectTriggerClass = "bg-bg";

/** Compact Select trigger matching filter field padding/size. */
export const compactFieldTriggerClass = `box-border min-w-0 ${selectTriggerClass}`;

/** Single-column stack used for every contacts filter block. */
export const contactStackClass = "flex min-w-0 flex-col gap-3";

/** Menu rows sized to match the compact field text, not the shared default. */
export function compactSelectItemClassName(state: {
  isFocused: boolean;
  isSelected: boolean;
}): string {
  return selectItemClassName(state, "sm");
}
