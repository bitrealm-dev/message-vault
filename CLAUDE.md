...
GitHub Pages custom domain and `docs/public/CNAME` must both be `bitrealm.io`. Putting `bitrealm.dev` in that file leaves the Pages certificate in `dns_changed` and HTTPS fails. Apex A records should keep pointing at GitHub Pages (`185.199.108–111.153`). Leave `api` / `app` / `cdn` DNS records alone.
