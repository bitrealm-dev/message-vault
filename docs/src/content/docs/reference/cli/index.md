...
These pages are Developer docs. The User Guide [Import](/import-from-a-backup/) chapter does not require these commands.
...
- `vault-push` imports a JSONL export folder into a running Message Vault. See [Extract to files](/how-to/extract-to-files/).
- `vault-pull` downloads messages from a running vault into a local JSONL folder. See [Export from the vault](/how-to/export-from-the-vault/).
