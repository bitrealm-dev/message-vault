...
Stop the other process. From a clone, `./scripts/run-vault-dev.sh` and a Compose stack both want port 8080. See [Operator Docker](/developer/docker-compose/) if two Compose files are fighting over that port.
