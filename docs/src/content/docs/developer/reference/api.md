...
(`bind`). Prefer the desktop app [Import](/user/import-from-a-backup/) screen or **`vault-push`** CLI for
day-to-day import; prefer [Export](/user/how-to/export-from-the-vault/) or **`vault-pull`** for download. This page
documents the HTTP surface those tools call. They send [JSONL](/developer/reference/export-structure/)
