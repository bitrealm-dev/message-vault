...
| Mode | Behavior |
|------|----------|
| **replace** | Wipe that source’s messages for the account, then reload. CLI `import` default (wipes each `export.source` found in the input directory, or the `--source` override). |
